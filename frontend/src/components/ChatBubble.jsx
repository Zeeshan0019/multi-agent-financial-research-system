...
export function AssistantBubble({ steps = [], answer, citations = [] }) {
  ...
  // Build a lookup from citation number -> citation object so inline [n]
  // markers in the answer text can be rendered as clickable source tabs.
  const citeById = {};
  citations.forEach((c, i) => { citeById[c.id ?? i + 1] = c; });

  // Split the answer on [n] tokens, turning each recognised marker into an
  // inline Citation while leaving the surrounding prose intact.
  function renderAnswerWithInlineCitations(text) {
    if (!text) return null;
    ...
    <div className="flex justify-start items-start gap-3 animate-fade-up">
      ...
      <div className="flex flex-col items-start max-w-2xl w-full">
        <div className="card rounded-2xl rounded-tl-sm p-5 border border-slate-200 bg-white shadow-sm w-full">
          ...
          {/* Main Answer — inline [n] markers become clickable source tabs */}
          <div className="text-sm leading-relaxed text-ledger-950 font-medium">
            <p className="whitespace-pre-line">{renderAnswerWithInlineCitations(answer)}</p>
            {citations.length > 0 && (
