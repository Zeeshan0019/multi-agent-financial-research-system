...
import { getChatHistory, askResearchAgent, clearChatHistory } from "../api/chat.js";
...
export default function Research() {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          answer: isTimeout
            ? "The Research Agent is taking longer than usual (embedding model loading). Please try asking again — subsequent questions will be much faster."
            : `Research Agent error: ${err?.response?.data?.detail || err?.message || "Please check the backend is running."}`,
          steps: [],
          citations: [],
        },
      ]);
  ...
  async function clearChat() {
    ...
    const scope = selectedDoc ? `the conversation for ${selectedDoc.company}` : "your entire research history";
    if (!window.confirm(`Clear ${scope}? This can't be undone.`)) return;
    setClearing(true);
    const prev = messages;
    setMessages([]); // optimistic clear
    try {
      await clearChatHistory(selectedDocId || undefined);
    } catch (err) {
      setMessages(prev); // roll back on failure
      window.alert("Couldn't clear the chat. Please try again.");
    } finally {
      setClearing(false);
    }
  }
          ...
          <div className="mb-3 flex items-center gap-3 border-b border-slate-100 pb-3">
            ...
            {messages.length > 0 && (
              <button
                ...
                onClick={clearChat}
                disabled={clearing}
                title={selectedDoc ? `Clear conversation for ${selectedDoc.company}` : "Clear all research history"}
                className="shrink-0 inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white hover:border-rose-300 hover:bg-rose-50 hover:text-rose-600 px-3 py-2 text-xs font-bold text-ledger-950/60 shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {clearing ? (
