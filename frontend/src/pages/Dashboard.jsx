...
import { getDocumentMetrics, getDocumentRedFlags, deleteDocument, reprocessDocument } from "../api/documents.js";
...
function DocumentRow({ doc, expanded, onToggle, onDelete }) {
  ...
  // Fetch when expanded and data not yet loaded
  useEffect(() => {
    if (!expanded) return;
    if (metrics !== null && flags !== null) return; // already loaded
    setDetailLoading(true);
    Promise.all([getDocumentMetrics(doc.id), getDocumentRedFlags(doc.id)])
      .then(([m, f]) => { setMetrics(m); setFlags(f); setDetailLoading(false); })
      .catch(() => setDetailLoading(false));
  }, [expanded, doc.id]); // eslint-disable-line
  ...
  async function handleReprocess(e) {
    ...
    try {
      ...
      const [m, f] = await Promise.all([getDocumentMetrics(doc.id), getDocumentRedFlags(doc.id)]);
      setMetrics(m);
      setFlags(f);
    } catch {
      // best-effort; leave existing view in place on failure
    } finally {
      setReprocessing(false);
    }
  }

  // Reset cache ONLY when status transitions from processing → ready (not on every render)
  useEffect(() => {
    if (prevStatusRef.current === "processing" && doc.status === "ready") {
      setMetrics(null);
      setFlags(null);
    }
              ...
              <div className="animate-fade-up space-y-6">
                ...
                <div>
                  <div className="mb-4 flex items-center justify-between">
                    ...
                    {metrics.citation && (
                      <Citation index="M" label={metrics.citation.label || companyName} page={metrics.citation.page} section={metrics.citation.section} />
                    )}
                  </div>
                  {!metrics.highlights || metrics.highlights.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {metrics.highlights.map((h, idx) => (
                        <MetricCard key={h.label} {...h} citationIndex={idx + 1} />
                      ))}
                    </div>
                  )}
                </div>
                      ...
                      {flags.map((f, i) => (
                        <div key={i} className="card card-hover px-5 py-4 border-l-[3.5px] border-l-rose-400 bg-white/60">
                          <div className="mb-2 flex flex-wrap items-center gap-2">
                            ...
                            {f.citation && <Citation index={i + 1} label={f.citation.label || companyName} page={f.citation.page} section={f.citation.section} snippet={f.citation.snippet} />}
                          </div>
