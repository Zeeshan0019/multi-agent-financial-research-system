...
# Groq deprecated several chat models (announced 2026-06-17); requests to them
# now return HTTP 404. If a stale .env still points at one, transparently remap
# it to the recommended replacement so extraction keeps working.
...
def _resolve_groq_model() -> str:
    ...
    if model in _DEPRECATED_GROQ_MODELS:
        ...
        logger.warning(
            "Groq model {old} is deprecated; using {new} instead",
            old=model, new=replacement,
        )
        return replacement
    return model
...
class ChunkFinancialFacts(BaseModel):
    ...
    @classmethod
    def validate_metrics_list(cls, value: Any) -> List[Any]:
        """Coerce to a list and drop empty/invalid elements. LLMs frequently
        append a trailing empty object (e.g. `[{...}, {}]`); without this,
        strict per-element validation would reject the ENTIRE extraction and
        discard perfectly good metrics, forcing a fake-data fallback."""
        items = cls._as_list(value)
        cleaned = []
        for item in items:
            if not isinstance(item, dict):
                continue
            # A metric is only usable if it has both a name and a value.
            has_name = safe_string(item.get("metric")) is not None
            has_value = safe_string(item.get("value")) is not None
    ...
    @classmethod
    def validate_company_info_list(cls, value: Any) -> List[Any]:
        items = cls._as_list(value)
        cleaned = []
        for item in items:
            if not isinstance(item, dict):
                continue
            has_field = safe_string(item.get("field")) is not None
            has_value = safe_string(item.get("value")) is not None
