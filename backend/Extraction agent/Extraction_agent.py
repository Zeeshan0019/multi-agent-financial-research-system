...
class Metric(BaseModel):
    ...
    value: Any = Field(description="Value exactly present in the chunk.")
...
class ChunkFinancialFacts(BaseModel):
    ...
    @classmethod
    def validate_list(cls, value: Any, info: ValidationInfo) -> List[Any]:
        if value is None or value == "":
            return []
        ...
        if not isinstance(value, list):
            return []
        # The LLM sometimes emits an item with a null/empty required field
        # (e.g. {"metric": "PAT", "value": null}) when it isn't confident
        # about a value, despite the prompt telling it to omit such items.
        # A single malformed item would otherwise fail validation for the
        # ENTIRE ChunkFinancialFacts object (all other, valid items included),
        # triggering a full retry/fallback. Drop only the malformed item here
        # instead, so the rest of the chunk's valid metrics still get through
        # on the first attempt.
        required_keys = ("metric", "value") if info.field_name == "metrics" else ("field", "value")
        filtered: List[Any] = []
        for item in value:
            if isinstance(item, dict) and any(safe_string(item.get(key)) is None for key in required_keys):
                logger.debug(
                    "Dropping malformed {field} item with missing required field(s): {item}",
...
def coerce_chunk_financial_facts(value: Any) -> ChunkFinancialFacts:
    ...
    if isinstance(value, ExtractedFinancialData):
        return legacy_financial_data_to_facts(
    ...
    if isinstance(sanitized, dict):
        ...
        wrapped = (
            sanitized.get("financial_data")
            if "financial_data" in sanitized
            else sanitized.get("data")
        )

        if (
            isinstance(wrapped, (dict, list))
            and not any(
                key in sanitized
                for key in (
                    "metrics",
        ...
        if any(
            ...
            payload = normalize_chunk_fact_payload(sanitized)

            # -------------------------------
            # Validate metrics one by one
            # -------------------------------
            valid_metrics = []
            ...
            # -------------------------------
            # Validate company_info one by one
            # -------------------------------
            valid_company_info = []

            for info in payload.get("company_info", []):

                try:
                    valid_company_info.append(
                        CompanyInfoFact.model_validate(info).model_dump(
                            mode="json",
                            exclude_none=True,
                        )
                    )
            ...
            return ChunkFinancialFacts(
                metrics=[
                    Metric.model_validate(x)
                    for x in valid_metrics
                ],
                company_info=[
                    CompanyInfoFact.model_validate(x)
                    for x in valid_company_info
                ],
            )
...
async def extract_financial_chunk(chunk: Dict[str, Any], metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if LLM_STRUCTURED_OUTPUT_ENABLED:
        try:
            ...
            logger.warning(
                "Structured fact extraction failed for chunk {chunk}: {error}",
                chunk=chunk.get("index"),
                error=compact_exception_message(structured_exc),
            )

            if is_llm_connection_failure(structured_exc):
                return local_fallback_chunk_result(chunk, structured_exc)

    # ---------------------------------------------------------
    # 2. Parser Output (Retry Once)
    # ---------------------------------------------------------
    for attempt in range(2):
        try:
            ...
            logger.exception(
            "Parser attempt {} failed for chunk {}",
            attempt + 1,
            chunk.get("index"),
            )

            print("\n" + "=" * 100)
            print("FULL PARSER EXCEPTION")
            print("=" * 100)
            print(parser_exc)
            print("=" * 100)
            ...
            logger.exception(
                "LLM extraction failed for chunk {chunk}",
                chunk=chunk.get("index"),
            )
    ...
    if LLM_RAW_REPAIR_ENABLED:
        ...
        try:
            ...
            logger.warning(
                "Raw repair failed for chunk {chunk}: {error}",
                chunk=chunk.get("index"),
                error=compact_exception_message(raw_exc),
            )

            primary_error = raw_exc

    # ---------------------------------------------------------
    # 4. Final Fallback
    # ---------------------------------------------------------
    return local_fallback_chunk_result(
        chunk,
        primary_error or RuntimeError("Financial extraction failed"),
    )
