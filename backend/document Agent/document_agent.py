...
class DocumentAgent:
    ...
    def process_document(
        ...
        try:
            ...
            self._index_chunks(chunks, session_id)
    ...
    def search(
        ...
        """Top-k most relevant chunks for a query, scoped to a session."""
        vector_store = self._load_session_index(session_id)
        ...
        for doc, score in results:
            if company_filter and doc.metadata.get("company_name") != company_filter:
                continue
            if document_id_filter and doc.metadata.get("document_id") != document_id_filter:
                continue
            hits.append(
                {
    ...
    def delete_document(self, session_id: str, document_id: str) -> bool:
        """Remove all chunks for a document from a session's index (rebuild-based)."""
        vector_store = self._load_session_index(session_id)
        if vector_store is None:
            return False
        ...
        if remaining:
            rebuilt = FAISS.from_documents(remaining, self.embeddings)
            rebuilt.save_local(str(index_path))
        else:
    ...
    def _index_chunks(self, chunks: List[LCDocument], session_id: str) -> None:
    ...
    def _load_session_index(self, session_id: str) -> Optional[FAISS]:
        index_path = self._session_index_path(session_id)
    ...
    def _session_index_path(self, session_id: str) -> Path:
        return self.config.VECTOR_STORE_ROOT / session_id
...
try:
    ...
    async def upload_document(
        ...
        save_dir = agent.config.UPLOAD_ROOT / session_id
        ...
        result = agent.process_document(
            file_path=str(dest_path), session_id=session_id, company_name=company_name,
        )
    ...
    async def delete_document(session_id: str, document_id: str):
        agent = get_document_agent()
        deleted = agent.delete_document(session_id, document_id)
        if not deleted:
...
if __name__ == "__main__":
    ...
    agent = DocumentAgent()
    result = agent.process_document(pdf_path, session_id=test_session, company_name=company)
    ...
    if result.document.status == "indexed":
        ...
        for hit in agent.search(test_session, "red flags in debt levels", k=3):
